...
[`PolynomialScheduler`](../../../../src/jacinta/utils/scheduler/polynomial_scheduler.py) is a [`Scheduler`](scheduler.md) that maps each depth level of an [`NDSpace`](../ndspace/nd_space.md) tree to a floating-point value according to a polynomial function. It provides a depth-dependent configuration mechanism for parameters that must evolve polynomially across different precision levels.
