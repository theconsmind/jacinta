...
[`ConstantScheduler`](../../../../src/jacinta/utils/scheduler/constant_scheduler.py) is a [`Scheduler`](scheduler.md) that maps every depth level of an [`NDSpace`](../ndspace/nd_space.md) tree to the same floating-point value. It provides a depth-independent configuration mechanism for parameters that should remain constant across all precision levels.
