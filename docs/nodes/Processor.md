...
$$
v_\mu = \Sigma^{-1} \cdot \mathrm{diff},
$$
...
$$
\mu \leftarrow \mu + \alpha_\mu \cdot r_{\mathrm{norm}} \cdot v_\mu.
$$
...
$$
\mathrm{diff} \cdot \mathrm{diff}^T,
$$

with the current covariance $\Sigma$. This comparison produces a matrix

$$
v_\Sigma = \mathrm{diff} \cdot \mathrm{diff}^T - \Sigma,
$$
...
$$
\Sigma \leftarrow \Sigma + \alpha_\Sigma \cdot r_{\mathrm{norm}} \cdot v_\Sigma
$$
